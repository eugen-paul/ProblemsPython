...
class SortedList(MutableSequence):
    ...
    def update(self, iterable):
        """Update sorted list by adding all values from `iterable`.`O(k*log(n))`"""
        _lists = self._lists
        _maxes = self._maxes
    ...
    def remove(self, value):
        """Remove `value` from sorted list; `value` must be a member. If `value` is not a member, raise ValueError."""
        _maxes = self._maxes
    ...
    def reverse(self):
        """Raise not-implemented error. Use ``reversed(sl)`` for an iterator over values in descending sort order."""
        raise NotImplementedError('use ``reversed(sl)`` instead')


    def islice(self, start=None, stop=None, reverse=False):
        """Return an iterator that slices sorted list from `start` to `stop`. The `start` and `stop` index are treated inclusive and exclusive, respectively."""
        _len = self._len
    ...
    def irange(self, minimum=None, maximum=None, inclusive=(True, True),
               reverse=False):
        """Create an iterator of values between `minimum` and `maximum`. Both `minimum` and `maximum` default to `None` which is automatically inclusive of the beginning and end of the sorted list."""
        _maxes = self._maxes
    ...
    def append(self, value):
        """Raise not-implemented error. Use ``sl.add(value)`` instead"""
        raise NotImplementedError('use ``sl.add(value)`` instead')


    def extend(self, values):
        """Raise not-implemented error. Use ``sl.update(values)`` instead"""
...
class SortedKeyList(SortedList):
    ...
    * :func:`SortedKeyList.irange_key`
    ...
    def remove(self, value):
        """Remove `value` from sorted-key list; `value` must be a member. If `value` is not a member, raise ValueError. """
        _maxes = self._maxes
